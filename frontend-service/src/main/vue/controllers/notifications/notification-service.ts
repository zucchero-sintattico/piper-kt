...
function createNotificationService() {
  ...
  function setupMessagesListeners() {
    notificationService.on(
      ...
      async (data: NewMessageOnDirectNotification) => {
        console.log(
          "NotificationService: NewMessageOnDirectNotification",
          data
        );
    ...
    notificationService.on(
      ...
      async (data: NewMessageOnChannelNotification) => {
        console.log(
          "NotificationService: NewMessageOnChannelNotification",
          data
        );
    ...
    notificationService.on(
      ...
      async (data: ServerDeletedNotification) => {
        ...
        if (appStore.selectedServer?.id == data.serverId) {
          console.log(
            "NotificationService: Refreshing servers and moving to directs"
          );
          appStore.setDirects();
  ...
  function setupUsersListeners() {
    notificationService.on(
      UserOnlineNotification.type,
      async (data: UserOnlineNotification) => {
    ...
    notificationService.on(
      UserOfflineNotification.type,
      async (data: UserOfflineNotification) => {
  ...
  function setupFriendsListeners() {
    ...
    notificationService.on(
      ...
      async (data: FriendRequestAcceptedNotification) => {
        console.log(
          "NotificationService: FriendRequestAcceptedNotification",
          data
        );
        await userStatusStore.reloadUserStatus(data.from);
        await friendsStore.refresh();
