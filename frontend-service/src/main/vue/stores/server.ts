...
export const useServerStore = defineStore(
  ...
  () => {
    ...
    async function getServerParticipants(serverId: string) {
      return (
        servers.value.find((server) => server.id === serverId)?.participants ??
        []
      );
    }
    ...
    async function deleteChannel(serverId: string, channelId: string) {
      const response = await channelController.deleteChannel({
        serverId,
        channelId,
      });
      if (response.statusCode === 200) {
    ...
    async function kickUser(serverId: string, username: string) {
      const response = await serverController.kickUserFromTheServer({
        serverId,
        username,
      });
      if (response.statusCode === 200) {
